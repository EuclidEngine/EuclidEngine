...
uniform float _KleinV = (float)(1 + 1e-4);
uniform float _Proj;
...
v2f vert(vin v) {
  ...
  if (_Enable > 0.0) {
    ...
    w_pos.xyz = w_pos.xyz * 1.0001;

    //Apply TanK stretching if applicable
    #if USE_TAN_HEIGHT
    w_pos.y = tan(w_pos.y) * sqrt(1.0 + K * dot(w_pos.xz, w_pos.xz));
    #endif

    //Convert from Klein coordinates to Poincaré coordinates
    w_pos.xyz /= sqrt(1 + dot(w_pos.xyz, w_pos.xyz)) + 1.0;
